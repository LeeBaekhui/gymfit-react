...
const FoodCalo = ({ searchValue }) => {
  ...
  return (
    <Table>
      <TableHead>
        <TableRow>
          <TableCell padding="checkbox">
            <Checkbox
              indeterminate={selected.length > 0 && selected.length < foodCalo.length}
              checked={foodCalo.length > 0 && selected.length === foodCalo.length}
      ...
      <TableBody>
        {foodCalo.length > 0 ? (
          foodCalo.map((article, index) => {
            const isItemSelected = isSelected(article["FOOD_NM_KR "]);
            return (
              <TableRow
                key={index}
                onClick={(event) => handleClick(event, article["FOOD_NM_KR "] )}
                role="checkbox"
